...
const mysqlRateLimiter = async (req, res, next) => {
  ...
  const windowStart = new Date(
    currentTime.getTime() - config.windowMilliseconds,
  );
  ...
  try {
    ...
    await connection.execute('DELETE FROM rate_limits WHERE window_start < ?', [
      windowStart,
    ]);
    ...
    const [existingRows] = await connection.execute(existingEntryQuery, [
      clientIP,
      windowStart,
    ]);
